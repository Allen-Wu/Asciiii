...
class ASCII:
    ...
    @classmethod
    def _hamming_dis(cls, a, b, gray_scale=False):
        if not gray_scale:
    ...
    @classmethod
    def hamming_match_static(cls, measure_grid, ascii_dict, eta, gray_scale=True):
        if measure_grid.mean() > 255 * (1 - eta):
            return ord(' ')
        if measure_grid.mean() < 255 * eta:
            return ord('#')

        id_max = 0
        distance_min = 256
        for id in ascii_dict:
            distance = ASCII._hamming_dis(ascii_dict[id], measure_grid, gray_scale)
            if distance < distance_min:
                distance_min = distance
                id_max = id
        return id_max
